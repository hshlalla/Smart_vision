...
데이터셋 준비 측면에서도 구조화 작업이 진행되었다. 현재 통합 데이터셋은 자동차 부품과 반도체 장비 부품을 포함한 `1491`개 항목으로 정리되어 있으며, `1192`개 train / `299`개 test split과 retrieval evaluation용 manifest가 이미 생성된 상태다. 따라서 일부 실험 수치는 아직 실행 중이더라도, evaluation input 자체는 ad hoc이 아니라 고정된 split 위에 구축되어 있다는 점을 최종본에 명시하는 것이 좋다.
...
새로운 평가 작업은 이제 단순 계획 수준을 넘었다. 현재 저장소에는 `experiments/` 아래 재현 실험 러너와 상태 요약 문서가 존재하며, 일부 실행은 다른 머신에서 이미 수행되었다. 다만 `experiments/runs/...` raw output은 local-output 성격이라 git에 항상 포함되지는 않을 수 있으므로, 최종 보고서에서는 raw path보다 `experiments/CURRENT_EXPERIMENT_STATUS.md`와 `submission/evidence/`로 복사된 요약 근거를 기준으로 삼는 것이 안전하다.
...
Retrieval effectiveness 측면에서는 과거 image-only baseline 외에도 두 가지 새로운 결과가 있다. 첫째, current-index sanity benchmark에서 `Accuracy@1 = 0.9732`, `Hit@5 = 1.0`, `MRR = 0.9855`가 관찰되었다. 둘째, isolated experimental collection에서 수행한 sampled holdout `C3` baseline은 `30`개 샘플, item당 held-out query image `1`장, `OCR off`, `reranker off` 조건에서 group `Hit@1 = 1.0`, group `Hit@5 = 1.0`, `MRR = 1.0`, exact `item_id@1 = 0.9667`를 기록했다. 다만 이 결과는 조심해서 써야 한다. current-index benchmark는 strict generalisation benchmark가 아니라 operational sanity evidence이며, sampled `C3`는 full controlled `C1/C2/C3/C4` 비교 전체를 대체하지는 않는다.
...
Latency evaluation도 이전보다 근거가 강해졌다. current-index suite에서는 warm total mean 약 `653.65 ms`, sampled `C3` baseline에서는 warm total mean 약 `731.13 ms`가 관찰되었다. sampled run 내부에서는 preprocessing warm mean이 약 `698.96 ms`였고, text search는 약 `20.39 ms`, image search는 약 `3.72 ms`였다. 이는 현재 local setup에서 latency 병목이 Milvus 검색보다 query-side preprocessing/embedding에 있음을 보여준다. 다만 `p50`, `p90`, `p95`와 resource summary는 아직 없으므로, latency study를 fully completed로 써서는 안 된다.

사용성 평가는 아직 최종 수치가 없는 영역이다. 다만 현재는 listing-oriented task와 post-task Google Form 기반 pilot protocol이 정의되어 있으므로, 최종 보고서에서는 “pilot protocol defined; aggregate outcomes pending”으로 표현하는 것이 적절하다.
