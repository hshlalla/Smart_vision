...
추가로 최근 구현에서는 인덱싱 경로 자체에도 review step이 강화되었다. 사용자가 이미지를 업로드하면 시스템은 즉시 저장하지 않고, 먼저 GPT 기반 metadata preview를 생성한 뒤 사용자가 필드를 수정하고 confirm하도록 설계되었다. 이 preview-confirm 흐름은 초기 draft 시점보다 human review 구현 수준을 한 단계 더 구체화한 변화이며, “완전 자동 등록”보다 listing assistance에 더 적합하다.
