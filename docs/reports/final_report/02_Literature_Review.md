...
실제 사용자 이미지는 cluttered background, partial occlusion, multi-object scene을 자주 포함한다. 따라서 실제 제품 영역이나 라벨 영역을 중심으로 보는 region-focused 처리의 필요성이 크다. 본 프로젝트는 현재 full detector pipeline을 중심으로 설계되지는 않았지만, 이후 region focus와 label-aware processing이 중요한 future work로 남아 있다.
