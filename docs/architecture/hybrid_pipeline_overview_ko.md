# Smart Vision 하이브리드 검색 파이프라인 정의서

> 현재 구현 기준의 런타임 source of truth는 `README.md`, `apps/api/README.md`, `packages/model/README.md`, `apps/web/README.md`다. 이 문서는 개념 설명용 요약이며, 최신 구현에서는 preview-confirm 비동기 인덱싱, duplicate candidate review, label-image assisted preview, 기존 모델 merge 정책, 경량 text-only search가 포함된다.

## 1. 프로젝트 전반 개요
- **목표**: 자동차 부품과 반도체 장비용 부품처럼 세부 식별이 어려운 중고 부품을 대상으로, 이미지와 텍스트 메타데이터를 결합한 멀티모달 검색/인덱싱 파이프라인을 제공한다.
- **핵심 컴포넌트**
  - `smart-vision-model`: 파이프라인 로직과 학습/전처리 모듈을 패키지화.
  - `smart-vision-api`: FastAPI 기반 REST 서비스로 오케스트레이터를 노출.
  - `smart-vision-web`: React 기반 메인 사용자 인터페이스.
  - `smart-vision-demo`: Gradio 기반 보조 디버그/데모 인터페이스.
- **데이터 저장소**: Milvus 2.4+ (벡터 + 속성 하이브리드 컬렉션), 이미지 사본은 로컬 `media/`.

## 2. 입력 파이프라인
### 2.1 데이터 소스
- **통합 부품 데이터셋**
  - 자동차 부품(`gparts`)과 반도체 장비용 부품(`1090481`)을 통합한 데이터셋을 사용한다.
  - 인덱싱 시 메타데이터는 `maker`, `part_number`, `category`, `description` 중심으로 정규화된다.
- **사용자 업로드 이미지**
  - 웹 인덱싱 경로는 사용자가 업로드한 다중 이미지를 입력으로 받는다.
  - preview 단계에서는 대표 이미지와 보조 라벨 이미지를 함께 사용해 메타 초안을 생성할 수 있다.

### 2.2 주요 입력 인터페이스
| 사용 시나리오 | 함수/엔드포인트 | 입력 형식 | 비고 |
|---------------|----------------|-----------|------|
| 단건 이미지 인덱싱 | `HybridSearchOrchestrator.preprocess_and_index` | 이미지 경로, `{"model_id", "maker", ...}` | 모델 ID 필수 |
| 트래커 기반 일괄 인덱싱 | `HybridSearchOrchestrator.index_tracker_model` | `model_id`, 이미지 루트 경로 | CSV에서 메타 자동 조인 |
| 커스텀 일괄 적재 | `HybridSearchOrchestrator.bulk_index` | `{"metadata": {...}, "images": [...]}` 배열 | `halt_on_error`로 실패 전략 제어 |
| REST 색인 API | `POST /api/v1/hybrid/index/preview` + `POST /api/v1/hybrid/index/confirm` + `GET /api/v1/hybrid/index/tasks/{task_id}` | JSON(base64 이미지 + 메타 필드) | preview에서 GPT 메타 초안과 duplicate candidate 확인 후, confirm은 비동기 큐로 저장 |

## 3. 전처리 및 임베딩 단계
### 3.1 메타데이터 정규화
- `MetadataNormalizer`가 `maker`, `part_number`, `category`, `description` 등을 표준화.
- 모델별 텍스트 설명은 `Maker`, `Part Number`, `Category`, `Description`을 조합해 생성한다.
- preview 단계에서 생성된 메타 초안은 confirm 전 사용자가 수정할 수 있으며, confirm 후 정규화된 값만 인덱싱된다.

### 3.2 이미지 & 텍스트 처리
- `PreprocessingPipeline`
  1. **OCR (`PaddleOCRVLPipeline`)**: 이미지에서 텍스트 추출 → 중복 라인 제거 후 누적 저장.
  2. **이미지 임베딩 (`Qwen3VLImageEncoder`)**: Qwen3-VL-Embedding-2B 기반 이미지 벡터 생성.
  3. **이미지 캡셔닝 (`GPTVLCaptioner` 또는 `Qwen3VLCaptioner`)**: 런타임 환경에 따라 GPT 또는 Qwen captioner 사용.
  4. **텍스트 임베딩 (`BGEM3TextEncoder`)**: OCR 텍스트와 캡션을 각각 BGE-M3로 인코딩.
- 출력: `image_vector`, `ocr_vector`, `caption_vector`, `ocr_tokens`, `ocr_text`, `caption_text`, 정규화된 `metadata`.

### 3.3 preview-confirm 인덱싱 흐름
- `preview`
  - 업로드 이미지(및 선택적 label image)를 사용해 GPT 기반 메타 초안을 생성한다.
  - duplicate candidate가 있으면 기존 모델 append 여부를 사용자가 검토할 수 있다.
- `confirm`
  - 확정된 메타데이터와 다중 이미지를 기준으로 비동기 인덱싱 작업을 큐에 넣는다.
  - 작업 상태는 `task_id` 기반 polling으로 추적한다.

### 3.4 중복 관리 및 기본 키
- 이미지 PK 패턴: `MODEL_ID::img_###`.
- 기존 PK 충돌 시 자동 증가 index로 재할당.
- 현재 구현에서는 동일 부품이 다시 입력될 때 `maker + part_number` 또는 `part_number` 기준으로 기존 모델을 재사용할 수 있다.
- interactive indexing에서는 preview 단계에서 duplicate candidate를 보여주고, 사용자가 새 모델 유지 또는 기존 모델 append를 선택한다.
- batch ingestion에서는 동일 이미지 경로를 skip하고, 새 이미지면 기존 모델에 이어 붙이며, 더 풍부한 `description`/`metadata_text`는 merge한다.
- 이미지 사본은 `MEDIA_ROOT`(기본 `media/`) 아래 `PK.png`로 저장.

### 3.5 전처리 파이프라인 시각화
`NormalizedRecord`는 아래 흐름의 최종 산출물로, 이미지/텍스트 임베딩·정규화 메타데이터·OCR 텍스트를 한 번에 묶어 하이브리드 인덱서에 전달한다. 덕분에 Milvus 컬렉션(`image_collection`, `text_collection`, `model_ID index`)과 API 계층에서 동일한 DTO를 그대로 재사용할 수 있다.

```mermaid
graph TD
    Input["Input:<br/>Image Path + Metadata"] --> OCR["PaddleOCR<br/>extract()"]
    OCR --> MetaNorm["MetadataNormalizer<br/>normalize()"]
    Input -. metadata .-> MetaNorm
    MetaNorm --> Caption["GPT / Qwen Captioner<br/>generate()"]
    Input -. image .-> Caption
    Input --> Vision["Qwen3-VL Image Encoder<br/>encode()"]

    OCR --> OCRText["OCR Text Cleanup"]
    OCRText --> MetaNorm
    MetaNorm --> TextEmbed["BGE-M3 encode_document<br/>(OCR + Caption)"]
    Caption --> TextEmbed

    Vision --> Record["NormalizedRecord"]
    TextEmbed --> Record
    OCRText --> Record
    Caption --> Record
    MetaNorm --> Record

    Record --> ImageCol["image_collection"]
    Record --> TextCol["text_collection"]
    Record --> ModelText["model_ID index"]
```

## 4. Milvus 하이브리드 인덱스 구조
| 컬렉션 | 주요 필드 | 설명 |
|--------|-----------|------|
| `image_parts` | `pk`, `vector` | 이미지 임베딩 (HNSW + COSINE) |
| `text_parts` | `pk`, `vector` | OCR 라벨 임베딩 (BGE-M3) |
| `caption_parts` | `pk`, `vector` | Qwen3 캡션 임베딩 (BGE-M3) |
| `attrs_parts` | `pk`, `model_id`, `maker`, `part_number`, `category`, `ocr_text(≤2048)`, `caption_text(≤2048)`, `image_path` | 속성/메타 데이터 |
| `model_texts` | `model_id`, `vector`, `metadata_text`, `ocr_text`, `caption_text`, `maker`, `part_number`, `category`, `description` | 모델 단위 텍스트 + 캡션 표현 |
- `HybridMilvusIndex`가 컬렉션 생성 → 인덱스 생성 → 로드까지 자동 실행.
- `upsert_model()`로 모델 텍스트 벡터 최신화, `fetch_attributes()`/`fetch_models()`로 메타 조회.
- `model_id` 카운터는 더 이상 Milvus 컬렉션에 저장되지 않으며, 현재는 로컬 SQLite namespace 기반 카운터를 사용한다.

## 5. 검색/결과 파이프라인
### 5.1 검색 질의 흐름
1. **이미지 질의**: Qwen3-VL 이미지 임베딩 → `image_parts`에서 ANN 검색 → 모델별 이미지 유사도 집계.
2. **OCR 유사도**: 질의 이미지에서 추출한 OCR 텍스트 → `text_parts` 검색 → 라벨 유사도 누적.
3. **캡션 의미 유사도**: GPT/Qwen 캡션 → `caption_parts` 검색 → 의미 기반 유사도 계산.
4. **사용자 텍스트 질의(선택)**: 입력 텍스트를 `model_texts`와 매칭해 OCR 가중치에 합산.
5. **퓨전 스코어**: `FusionWeights(alpha, beta, gamma)` 기준 `α·image + β·ocr + γ·caption`.
6. **Lexical Boost**: 질의 문자열이 메타/캡션/OCR에 포함되면 +0.25 보정.
7. **Part Number 필터**: 요청에 `part_number`가 있으면 일치 모델만 반환하며 `verified=True`.
8. **Reranker(선택)**: 환경에 따라 `Qwen3-VL-Reranker-2B`를 사용할 수 있으나, 현재는 latency 비용을 고려해 optional path로 둔다.

### 5.2 API/데모 출력 포맷
```json
{
  "model_id": "SG-12345",
  "score": 0.87,
  "image_score": 0.82,
  "ocr_score": 0.71,
  "caption_score": 0.24,
  "text_query_score": 0.00,
  "maker": "SmartVision",
  "part_number": "PN-001",
  "category": "ETCH",
  "description": "...",
  "metadata_text": "...",
  "ocr_text": "...",
  "caption_text": "...",
  "images": [
    {
      "image_id": "SG-12345::img_001",
      "similarity": 0.84,
      "image_path": "/app/media/SG-12345::img_001.png",
      "caption_text": "Front panel of the SG-12345 etch chamber with control labels."
    }
  ],
  "verified": true
}
```

### 5.3 REST 검색 인터페이스
| 엔드포인트 | 입력 | 주요 옵션 | 응답 |
|------------|------|-----------|------|
| `POST /api/v1/hybrid/search` | JSON `{ "query_text", "image_base64", "part_number", "top_k" }` | 텍스트/이미지 동시 질의, PN 필터 | 상위 `top_k` 후보 목록 |
| `POST /api/v1/hybrid/index/preview` | JSON `{ image_base64_list, metadata_mode, label_image_base64_list }` | GPT 메타 초안 생성, duplicate candidate 확인 | preview response |
| `POST /api/v1/hybrid/index/confirm` | JSON `{ image_base64_list, maker, part_number, category, description, ... }` | confirm 후 비동기 인덱싱 작업 큐 등록 | `{"status": "queued", "task_id": "...", "model_id": "..."}` |
| `GET /api/v1/hybrid/index/tasks/{task_id}` | path parameter | 작업 상태 polling | `queued/running/succeeded/failed` |

## 6. 운영 및 확장 고려 사항
- **리소스**: 최초 실행 시 BGE/PaddleOCR 모델 가중치 다운로드 → GPU 메모리 확보 필요.
- **캡셔닝 정책**: hosted/default 모드에서는 GPT captioner, local mode에서는 Qwen captioner를 우선 사용한다.
- **텍스트 길이 관리**: `attrs_parts.ocr_text`/`caption_text`는 2048자까지 저장하며, 초과 시 안전하게 잘라냅니다.
- **Milvus 스키마 변경**: 변경 시 기존 컬렉션 드롭 후 오케스트레이터 재생성.
- **증분 인덱싱**: 기존 모델에 새로운 이미지 추가 시 OCR 텍스트는 중복 없이 병합.
- **로그/모니터링**: `smart-vision-api/logs/`에 서비스 로그 저장, 로거 레벨은 `LOG_LEVEL` 환경변수로 제어.
- **배포 경로**: `scripts/run_docker.sh`(Milvus 포함), `run_dev.sh`, `run_prod.sh` 스크립트 제공.
- **Text-only search**: chat/agent와 텍스트 검색 요청은 필요 시 경량 `BGE-M3 + model collection` 경로를 사용한다.

## 7. PT 활용 포인트
- 첫 장: 전체 아키텍처 다이어그램(데이터 수집 → 전처리 → Milvus → 검색/API)을 한 슬라이드에 정리.
- 중간 장: preview-confirm 인덱싱과 duplicate-aware ingestion 정책을 강조.
- 후반 장: 검색 흐름과 출력 예시를 JSON+UI 스크린샷으로 전달.
- 마무리: 운영상 주의사항(모델 다운로드, GPU, Milvus/SQLite 카운터, reranker 비용)과 향후 확장 소개.
