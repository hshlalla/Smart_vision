...
가장 어려운 기술적 문제는 단순히 “이미지 검색을 만드는 것”이 아니었다.
실제로는 **시각적 유사성**, **작고 노이즈가 많은 텍스트 단서**, **open-world retrieval**을 함께 다루면서도 OCR이 불안정할 때 시스템이 여전히 유용하도록 만드는 것이 핵심 과제였다.
...
초기 계획이 효과적이었던 이유는 두 가지다. 첫째, retrieval-first라는 전체 비전은 프로젝트 전반에 걸쳐 흔들리지 않았다. 둘째, frontend/API/model의 모듈 분리가 나중에 catalog retrieval과 agent orchestration을 추가할 때도 확장성을 제공했다.

하지만 개선할 점도 분명했다. 특히 evaluation planning과 timing이 더 좋았어야 했다. hindsight로 보면 retrieval, CER, latency benchmark 자동화를 더 초기에 넣었어야 한다. 목표 자체가 완전히 바뀌지는 않았지만, 표현은 더 정교해졌다. 처음에는 “AI part identification” 같은 넓은 표현이 가능했지만, 프로젝트가 진행되면서 이를 “evidence-aware shortlist assistant”라는 더 방어적이고 정확한 주장으로 다듬게 되었다.

## Q2. Technical challenge and solution

### (a) 가장 중요한 기술적 도전과제는 무엇이었는가?

가장 중요한 기술적 도전과제는 서로 약하고 성격이 다른 여러 신호를 하나의 유용한 retrieval system으로 통합하는 일이었다. 이 도메인에서는 두 부품이 전체 외형은 비슷하지만 작은 텍스트 차이로 구분되는 경우가 많다. 그런데 그 텍스트는 blur, glare, 작은 글씨 때문에 OCR이 실패하기 쉽다. 따라서 pure image similarity도 부족하고, pure OCR도 부족하다.

나는 이것이 프로젝트의 핵심 도전과제라고 본다. 왜냐하면 이 문제를 해결하지 못하면 실제 사용자에게 유용한 shortlist를 제공할 수 없고, 결국 프로젝트 목표 자체를 달성할 수 없기 때문이다.

### (b) 이를 해결하기 위해 어떤 접근을 취했는가?

이를 해결하기 위해 hybrid retrieval pipeline을 설계했다. 시스템은 사용자 이미지와 metadata를 받아 preprocessing, OCR, embedding 생성, 다중 Milvus collection 검색, weighted fusion, ranking을 수행한다. 또한 caption 기반 retrieval과 metadata-aware ranking도 지원한다. 이후에는 Qwen3-VL 기반 image retrieval과 reranking 쪽으로 설계를 확장하면서도, BGE-M3와 OCR evidence는 필요시 유지하는 mixed strategy도 고려했다.

또한 직접 검색만으로 부족한 경우를 위해 catalog retrieval과 agent orchestration도 추가했다. 안정성 측면에서는 writeback을 기본 opt-in으로 바꾸고, regression test와 latency instrumentation을 추가했다.

### (c) 그 접근의 효과를 평가하라.

이 접근은 효과적이었다. 가장 큰 이유는 문제의 실제 복잡성을 반영하는 working prototype을 만들었기 때문이다. 단순한 image-only framing보다 hybrid evidence가 왜 필요한지를 보여줬고, uncertainty와 user review를 설계에 반영했다는 점에서 진전이 있었다.

하지만 이 접근이 완전히 검증되었다고 볼 수는 없다. architecture 측면의 타당성은 잘 드러났지만, quantitative completeness는 아직 부족하다. 만약 다시 한다면 hybrid design 자체는 유지하되, 각 signal의 기여를 더 엄밀하게 보여주는 controlled ablation과 benchmark automation을 더 일찍 넣었을 것이다.
...
가장 중요한 조언은 **claim boundary를 정직하게 유지하라**는 것이다. 기능을 추가하는 속도보다, 그 기능이 실제로 어떤 가치를 주는지 평가하는 속도를 더 중요하게 봐야 한다. 아키텍처는 modular하게 유지하고, data version과 collection schema, artifact generation을 꼼꼼히 관리해야 한다. 또한 OCR은 useful but uncertain evidence로 취급해야 한다. 가장 큰 함정은 시각적으로 그럴듯한 retrieval 결과가 나온다고 해서, 그것이 충분히 신뢰 가능한 identification이라고 착각하는 것이다.
...
literature review에는 프로젝트의 핵심 문제와 해결 방향에 직접적인 영향을 준 자료를 넣어야 한다. 예를 들어 image retrieval, OCR의 한계, multimodal embedding, vector search, industrial part search, marketplace assistance 같은 문헌은 직접적으로 설계와 요구사항에 연결되므로 review에 포함하는 것이 적절하다.

반대로 implementation 과정에서 참고한 low-level package documentation, narrow troubleshooting source, 또는 단순 사용법 문서 같은 것은 개발에는 도움이 되었더라도 literature review의 핵심 내용으로는 적절하지 않다. literature review는 프로젝트의 개념적 기반을 설명하는 곳이지, 모든 참고자료를 나열하는 곳이 아니기 때문이다.
...
나는 중고 부품 식별 지원을 위한 working end-to-end prototype을 달성했다. 이 시스템은 hybrid indexing/search, evidence-aware retrieval, catalog lookup, agent-assisted path를 포함하는 모듈형 구조를 가진다.

### (ii) 가장 잘한 부분은 무엇인가?

가장 잘한 부분은 retrieval-first system framing 자체라고 생각한다. 이 문제를 standard classifier로 억지로 맞추지 않고, 실제 도메인 특성에 맞는 open-world retrieval 문제로 재구성했다는 점이 가장 큰 성과다. 이는 기술적 구현과 개념적 정당화가 동시에 잘 맞아떨어진 부분이다.

### (iii) 가장 부족한 부분과 개선 방향은 무엇인가?

가장 부족한 부분은 architecture보다 evaluation completeness라고 생각한다. 특히 aggregate CER benchmarking, latency percentile reporting, retrieval variant ablation이 아직 충분히 성숙하지 않다. 이를 개선하려면 benchmark automation을 더 이른 시점에 구축하고, 평가 파이프라인을 implementation의 핵심 일부로 다뤘어야 한다.
