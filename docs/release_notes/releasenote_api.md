...
### Added
- 인덱싱된 이미지 사본을 API에서 직접 제공할 수 있도록 `/media` 정적 서빙을 추가했습니다.
- 검색 결과 카드가 저장된 `image_path`를 썸네일로 바로 표시할 수 있는 UI 연동 경로를 마련했습니다.

### Changed
- 하이브리드 검색 백엔드가 최신 모델 스택 정리에 맞춰 `Qwen3-VL` 이미지 검색 + `BGE-M3` 텍스트 검색 + multimodal reranking 방향으로 문서/의존성 기준을 갱신했습니다.
- 카탈로그 검색 경로는 텍스트 임베딩을 `BGE-M3`로 유지하도록 재정렬했습니다.

### Fixed
- PaddleOCR 3.x 환경에서 `cls=True` 인자 불일치로 카탈로그 OCR 경로가 실패할 수 있던 문제를 완화했습니다.
- 표준 OCR 호출 실패 시 `predict()` fallback을 사용하도록 보강해 환경별 호환성을 높였습니다.
